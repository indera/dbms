...
const prepareLoanData = (
    ...
    data.forEach(value => {
        if (undefined === loanSumListByDistrictIdMap.get(value.district_id) || loanSumListByDistrictIdMap.get(value.district_id) === null) {
            loanSumListByDistrictIdMap.set(value.district_id, new Map());
        }
        loanSumListByDistrictIdMap.get(value.district_id).set(value.month, value.loan_sum);
    ...
    loanSumListByDistrictIdMap.forEach((loanSumByDateMap, districtIdkey) => {
        ...
        categories.forEach(monthVal => {
            districtLoanSumTimeSeries.push((loanSumByDateMap.get(monthVal) === undefined) ? null : (loanSumByDateMap.get(monthVal)));
        });
...
const Trend6: React.FC = () => {
    ...
    const urlCreditCardsIssuedByMonth = "/api/getTotalAmountOfCreditCardsIssuedPerMonth";
    const [dataLoansByDistrict, setDataLoansByDistrict] = useState(defaultFetchData);
    const [dataCreditCardsIssuedByMonth, setDataCreditCardsIssuedByMonth] = useState(defaultFetchData);
    ...
    const chartDataCreditCardsIssuedByMonth = prepareCardData(dataCreditCardsIssuedByMonth.rows);
    ...
    return (
        <>
            ...
            {!isLoading && dataLoansByDistrict && (
                <HighchartsReact highcharts={Highcharts} options={optionsLoansByDistrict}/>
            )}

            <Divider/>

            {!isLoading && dataLoansByDistrict && (
                <HighchartsReact highcharts={Highcharts} options={optionsCreditCardsIssuedByMonth}/>
            )}
        </>
    );
};
